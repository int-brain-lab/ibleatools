...
def main():
    ...
    try:
        ...
        print("[model] not found - checking local versions")
        try:
            base_ckpt = torch.load(Path(f'../encoding_models/{cfg.vintage}/SE_model_{cfg.vintage}.pt'), map_location=device)
            conf_ckpt = torch.load(Path(f'../encoding_models/{cfg.vintage}/Confidence_model_{cfg.vintage}.pt'), map_location=device)
            base_model.load_state_dict(base_ckpt["model_state"])
            conf_model.load_state_dict(conf_ckpt["model_state"])

        except:
            raise "could not find a version of SE model"
