...
def load_channel_release(
    *,
    ...
    """Resolve a released channel model through this branch's ``load_pretrained``.

    Swapped from his release registry to ``examples/figures/_release.channel_release``; returns
    the same dict the figure body consumes. ``registry_root``/``hf_token`` are accepted for
...
class RunConfig:
    ...
    # Kept for call-site compatibility; unused now that loading goes through load_pretrained.
