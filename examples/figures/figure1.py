...
def load_channel_interpolation_model_from_registry(
    *,
    ...
    release_config : dict
    """
    # Swapped from his release registry to this branch's load_pretrained (via
    # examples/figures/_release.channel_release). It returns the constructed encoder with its
    # normalisation buffers already populated, so the manual checkpoint/architecture handling that
    # was here is no longer needed; the rest of the figure is unchanged.
    rel = channel_release(hf_repo_id, vintage, device=str(device))
    if list(rel.features) != list(FEATURE_LIST):
        print("[figure 1] warning: release feature order differs from FEATURE_LIST")
    model = rel.model.to(device)

    print(f"[figure 1] loaded channel model vintage={vintage}")
    print(f"[figure 1] release directory: {rel.dir}")

    return model, rel.dir, rel.features, rel.config
