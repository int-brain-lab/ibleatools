...
class Config:
    ...
    # Released model: set hf_repo_id to your HF repo (or a local model directory). registry_root
    # is accepted for call-site compatibility and unused now that loading goes through
    # load_pretrained.
    registry_root: Path | None = None
    hf_repo_id: str | None = "int-brain-lab/ea-encoder-channel"
    ...
    ablation_checkpoint_root: Path = Path("ablation_checkpoints")
...
def _resolve_verified_release(cfg):
    ...
    # examples/figures/_release.channel_release), which verifies checksums on load. Returns the
    # same artifacts the body consumes; context lives at context_dir (root here, not a subdir).
    rel = channel_release(cfg.hf_repo_id, cfg.vintage, device=str(cfg.device))
    return (
        rel.dir,
        rel.context_dir,
        rel.config,
...
def load_released_combined_model(
    *,
    ...
    ckpt_path = release_dir / "spatial_encoder.pt"
