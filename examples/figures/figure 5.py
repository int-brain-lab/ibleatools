...
# Loader swapped to this branch's publishing system (examples/figures/_release.py); the figure
...
def compute_single_alignment_metrics(
    *,
    ...
    """
    Compute Figure 5 evaluation quantities directly from the alignment that was
    ...
    print("\n[Figure 5 single-alignment evaluation]")
...
def evaluate_heldout_test_set_alignments(
    *,
    ...
    probe and aggregate the per-probe metrics used in Figure 5 panel c.
    ...
    print(f"\n[Figure 5] aligning {n_total} held-out test probes...")

    for i, pid in enumerate(test_pids, start=1):
        print(f"[Figure 5 test alignment] {i}/{n_total} PID={pid}")
        ...
        try:
            ...
            print(f"[Figure 5] WARNING: alignment failed for PID={pid}: {exc}")
            if fail_fast:
                raise
    ...
    print("\n[Figure 5 held-out alignment summary]")
...
def _apply_release_config_to_cfg(cfg, release_config: dict) -> None:
    ...
    identifiers. This prevents Figure 5 from silently using settings that differ
...
def resolve_alignment_release(
    *,
    ...
    """
    Resolve Figure 5's released channel models.
    ...
    """
    # Loader swapped to this branch's publishing system (examples/figures/_release.py). It returns
    # the same artifacts his registry did -- config, features, split, preprocessing stats, and the
    # local release directory -- so the body below (checkpoint loading, model construction) is
    # unchanged, except that the weights now sit at the release-dir root, not under models/channel.
    rel = channel_release(cfg.hf_repo_id, cfg.vintage, device=str(device))
    release_dir = rel.dir
    release_config = rel.config
    release_features = rel.features
    split_manifest = rel.split
    preprocessing_stats = rel.stats

    _apply_release_config_to_cfg(cfg, release_config)

    base_path = release_dir / "spatial_encoder.pt"
    conf_path = release_dir / "confidence_model.pt"
    ...
    print(f"[figure 5] resolved release vintage={cfg.vintage}")
    print(f"[figure 5] release directory: {release_dir}")
...
class RunConfig:
    ...
    # Released-model interface. Set hf_repo_id to your HF repo (or a local model directory);
    # registry_root is accepted for call-site compatibility and unused now that loading goes
    # through load_pretrained.
...
def main():
    ...
    # ==============================================================
    # Resolve exact released model: local registry first, then HF tag
    # revision=<vintage>. The release determines the data identifiers,
    # PCA context, preprocessing statistics, split, and architecture.
    # ==============================================================
    ...
    # Context PCA volumes are loaded from the release itself.
    ...
    # Use the data identifiers saved with the released model.
    ...
    # Rebuild loaders using the frozen release split and preprocessing stats.
    (
        train_loader,
        conf_train_loader,
        val_loader,
    ...
    # Build directly from architecture metadata stored in the released
    # checkpoints; no hard-coded architecture assumptions are needed here.
    base_model, conf_model = construct_released_models(
        release=release,
        device=device,
    )

    # Sanity checks against the reconstructed data pipeline.
    if int(base_model.e_mean.numel()) != int(e_mean.numel()):
        raise RuntimeError(
            "Released model/data feature dimension mismatch: "
            f"model={base_model.e_mean.numel()}, data={e_mean.numel()}."
        )
    if list(release["features"]) != list(FEATURE_LIST):
        raise RuntimeError("Released feature order does not match FEATURE_LIST.")
    ...
    # --------------------------------------------------------------
    # Panels a/b use TARGET_PID as the example alignment.
    # Panel c is evaluated on EVERY held-out test PID stored in the
    # versioned split manifest that came with the Hugging Face release.
    # --------------------------------------------------------------
    brain_atlas = AllenAtlas()

    test_pids = _extract_test_pids_from_split_manifest(
        release["split_manifest"],
        all_pid_names=pid_names,
    )
    print(
        f"[Figure 5] released held-out test set contains "
        f"{len(test_pids)} PIDs."
    )

    metrics = evaluate_heldout_test_set_alignments(
        test_pids=test_pids,
        pid_names=pid_names,
        ephys=ephys,
        probe_positions=probe_positions,
        ...
        mapping="Cosmos",
        fail_fast=cfg.fail_fast_test_alignment,
    )

    plot_auto_alignment_figure(
        debug=debug,
        model=base_model,
        feature_list=release["features"],
        brain_atlas=brain_atlas,
        save_path="figure5_auto_alignment.pdf",
        cosmos_accuracy=metrics["region_acc_cosmos"],
        alignment_distance_um=metrics["mean_abs_channel_error_um"],
        acc_good=metrics["acc_good"],
        acc_suspicious=metrics["acc_suspicious"],
    )
