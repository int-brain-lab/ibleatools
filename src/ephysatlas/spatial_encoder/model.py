...
class NeighborEncoder(nn.Module):
    def __init__(self, f_ephys, f_region, d_model, d_pos=64, drop=0.1):
        ...
        self.embed = mlp(
            f_ephys + f_region + d_pos, d_model, d_model, n_layers=2, drop=drop
        )
...
class QueryEncoder(nn.Module):
    def __init__(self, f_ctx, f_region, d_model, d_pos=64, drop=0.1):
        ...
        self.pos = PosEnc3D(d_pos)
        self.embed = mlp(
            f_ctx + f_region + d_pos, d_model, d_model, n_layers=2, drop=drop
        )
        ...
        self.blocks = nn.ModuleList(
            [
                CrossBlock(
                    ...
                    nhead=nhead,
                    drop=drop,
                    neighbor_self_attn=neighbor_self_attn,
                )
...
# ---------- losses & training ----------
def gaussian_nll(
    mu_std: torch.Tensor, logvar: Optional[torch.Tensor], y_std: torch.Tensor
):
...
def train_hybrid(
    ...
    *,
    ...
    lr_scheduler: Optional[
        object
    ] = None,  # e.g., torch.optim.lr_scheduler.ReduceLROnPlateau
):
    """
    ...
    for ep in range(1, epochs + 1):
        ...
        for step, batch in enumerate(train_dl, 1):
            ...
            with torch.amp.autocast(device_type=device_type, enabled=use_grad_scaler):
                ...
                mask_dropped[
                    np.random.permutation(np.arange(mask.shape[0]))[
                        : int(ephys_drop * len(mask))
                    ]
                ] = False
                h_q, mu, logvar = get_query_repr_and_pred(
                    model, ctx_q, reg_q, p_q, e_n, reg_n, p_n, mask_dropped
                )
        ...
        if val_dl is not None:
            ...
            with (
                torch.no_grad(),
                torch.amp.autocast(device_type=device_type, enabled=use_grad_scaler),
            ):
                for batch in val_dl:
                    (
                        ctx_q,
                    ...
                    h_q, mu, logvar = get_query_repr_and_pred(
                        model, ctx_q, reg_q, p_q, e_n, reg_n, p_n, mask
                    )
                    val_sup = (
                        gaussian_nll(mu[has_ephys], logvar[has_ephys], y_e[has_ephys])
                        if (
                            heteroscedastic and (logvar is not None) and has_ephys.any()
                        )
            ...
            if lr_scheduler is not None:
                try:
                    ...
                    lr_scheduler.step(
                        current_val if current_val is not None else val_sup_mean
                    )
                except TypeError:
...
def evaluate_r2_per_feature(
    ...
    with torch.amp.autocast(device_type=device_type, enabled=use_autocast):
        for batch in tqdm(test_dl):
            ...
            h_q, mu_std, _ = get_query_repr_and_pred(
                model, ctx_q, reg_q, p_q, e_n, reg_n, p_n, mask
            )
            # unstandardize
