...
def LoadInsertionData(
    ...
    if raw_data:
        df_features = pd.read_parquet(
            path_data / f"{project}/{VINTAGE}/{agg}/raw_ephys_features_denoised.pqt"
        )
        channels = pd.read_parquet(
            path_data / f"{project}/{VINTAGE}/{agg}/channels.pqt"
        )
    else:
    ...
    for pid, df_pid in df_features.groupby(level="pid"):
        ...
        if raw_data:
            channel_indices = (
                channels.loc[pid].index.get_level_values("channel").to_numpy()
            )
            xyz_values = channels.loc[pid][["x", "y", "z"]].values
            xyz_planned_values = channels.loc[pid][
...
def build_channels_plus_emptyvoxels_with_neighbors(
    ...
    *,
    ...
    I_tr, I_va, I_te = (
        indices[:n_tr],
        indices[n_tr : n_tr + n_va],
        indices[n_tr + n_va :],
    )

    # ----- Per-channel voxel key using the SAME rounding used elsewhere -----
    xi_all, zi_all, yi_all = downsample_keys_from_xyz(
        ctx_manager, rec_xyz.numpy()
    )  # arrays length Nc
    ...
    vox_count_all = np.array(
        [max(1, train_key_counts.get(k, 0)) for k in rec_keys], dtype=np.float32
    )
