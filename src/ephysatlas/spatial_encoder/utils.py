...
def LoadInsertionData(
    ...
    for pid, df_pid in df_features.groupby(level="pid"):
        C = df_pid["x"].shape[0]
        # --- Prepare channel xyz (actual + planned), preserving the up->down reversal ---
        ...
        # Keep the final reversal (up->down)
...
def region_ids_from_xyz(
    ...
    mapping: "Cosmos" or "Allen" (must be supported by :func:`xyz_to_region_ids`)
    Returns: np.ndarray [C] of region IDs (int)
    """
    idx = brain_atlas.bc.xyz2i(xyz_m_np, mode=mode)  # meters → voxel indices
    # Delegates to xyz_to_region_ids(idx, brain_atlas, mapping=...) below
...
def build_channels_plus_emptyvoxels_with_neighbors(
    ...
    *,
    ...
    # Context stats over ALL grid voxels (rec + non-rec), as at training time
    ...
    if split_manifest is not None:
        # Use the model's authoritative saved split: map its train/val/test
    ...
    # Optionally hand back the stats actually used, for callers that need to record what
    # preprocessing ran. Appended (not inlined) so the default return shape is stable.
