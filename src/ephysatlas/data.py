...
# CCF grid dimensions (nx, ny, nz) by voxel resolution, matching the volumes already
# released under aggregates/atlas/encoding_volumes/ (see docs/source/how-to/load-encoding-volume.rst).
ENCODING_VOLUME_GRID_SHAPE_UM = {25: (456, 528, 320), 50: (228, 264, 160)}


def convert_h5_encoding_volume_to_npz(h5_path, npz_path):
    """Convert a sparse row-list encoding volume (.h5) to the legacy dense .npz schema.

    The .h5 format (produced by the ea-encoder-channel model repo) stores one row per
    in-brain, per-hemisphere voxel with an explicit ``ccf_xyz_index``. The legacy .npz
    format expected by :func:`download_encoding_volume` stores a dense
    ``(nx, ny, nz, n_features)`` grid with zeros outside the brain mask.

    Parameters
    ----------
    h5_path : Path
        Local path to the source .h5 encoding volume.
    npz_path : Path
        Local path the converted .npz file is written to.

    Returns
    -------
    Path
        ``npz_path``, for chaining into an upload step.
    """
    import h5py

    with h5py.File(h5_path, "r") as f:
        res_um = int(f.attrs["atlas_resolution_um"])
        ccf_xyz_index = f["ccf_xyz_index"][:]
        predicted_features = f["predicted_features"][:]
    ...
    grid_shape = ENCODING_VOLUME_GRID_SHAPE_UM[res_um]
    if (
        ccf_xyz_index[:, 0].max() >= grid_shape[0]
        or ccf_xyz_index[:, 1].max() >= grid_shape[1]
        or (ccf_xyz_index[:, 2].max() >= grid_shape[2])
    ):
        raise ValueError(
            f"ccf_xyz_index exceeds expected grid shape {grid_shape} for res_um={res_um}"
        )

    n_features = predicted_features.shape[1]
    ephys_atlas_vol = np.zeros((*grid_shape, n_features), dtype=np.float16)
    ephys_atlas_vol[
        ccf_xyz_index[:, 0], ccf_xyz_index[:, 1], ccf_xyz_index[:, 2], :
    ] = predicted_features

    np.savez_compressed(
        npz_path,
        ephys_atlas_vol=ephys_atlas_vol,
        feature_names=feature_names,
        mean_per_feature=predicted_features.mean(axis=0).astype(np.float32),
        std_per_feature=predicted_features.std(axis=0).astype(np.float32),
        grid_shape=np.array(grid_shape, dtype=np.int32),
        res_um=np.array([res_um], dtype=np.int32),
    )
    return npz_path
