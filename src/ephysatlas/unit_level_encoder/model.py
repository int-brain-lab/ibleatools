...
class ConvEncoder(nn.Module):
    def __init__(self, latent_dim: int):
        ...
        self.features = nn.Sequential(
            nn.Conv2d(1, 32, (3, 7), padding=(1, 3)), nn.GroupNorm(4, 32), nn.GELU(),
            nn.Conv2d(32, 64, (3, 5), stride=(1, 2), padding=(1, 2)), nn.GroupNorm(8, 64), nn.GELU(),
            nn.Conv2d(64, 128, (3, 5), stride=(2, 2), padding=(1, 2)), nn.GroupNorm(8, 128), nn.GELU(),
            nn.Conv2d(128, 256, (3, 5), stride=(2, 2), padding=(1, 2)), nn.GroupNorm(16, 256), nn.GELU(),
            nn.AdaptiveAvgPool2d((5, 8)),
        )
        self.head = nn.Sequential(
            nn.Flatten(), nn.Linear(256 * 5 * 8, 512), nn.GELU(), nn.Linear(512, latent_dim)
        )
...
class WaveformDecoder(nn.Module):
    def __init__(self, latent_dim: int, output_shape: Tuple[int, int]):
        ...
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(256, 128, (3, 4), stride=(1, 2), padding=(1, 1)), nn.GroupNorm(8, 128), nn.GELU(),
            nn.ConvTranspose2d(128, 64, 4, stride=2, padding=1), nn.GroupNorm(8, 64), nn.GELU(),
            nn.ConvTranspose2d(64, 32, 4, stride=2, padding=1), nn.GroupNorm(4, 32), nn.GELU(),
...
class ACGDecoder(nn.Module):
    def __init__(self, latent_dim: int, output_shape: Tuple[int, int]):
        ...
        self.fc = nn.Sequential(
            nn.Linear(latent_dim, 512), nn.GELU(), nn.Linear(512, 64 * rows * 26), nn.GELU()
        )
        self.refine = nn.Sequential(
            nn.ConvTranspose2d(64, 32, (3, 5), stride=(1, 2), padding=(1, 2), output_padding=(0, 1)),
            nn.GroupNorm(4, 32), nn.GELU(),
            nn.ConvTranspose2d(32, 16, (3, 5), stride=(1, 2), padding=(1, 2), output_padding=(0, 1)),
    ...
    def forward(self, z: torch.Tensor) -> torch.Tensor:
        ...
        if x.shape[-1] != time:
            x = F.interpolate(x[:, None], size=(rows, time), mode="bilinear", align_corners=False)[:, 0]
        return F.softplus(x)
...
class SharedProjector(nn.Module):
    def __init__(self, dim: int, hidden: int, out: int):
        ...
        self.net = nn.Sequential(
            nn.Linear(dim, hidden), nn.BatchNorm1d(hidden), nn.GELU(), nn.Linear(hidden, out)
        )
...
class MultimodalAutoencoder(nn.Module):
    ...
    def __init__(self, cfg: Config):
        ...
        self.wave_projector = SharedProjector(d, cfg.projector_hidden_dim, cfg.projector_dim)
        self.acg_projector = SharedProjector(d, cfg.projector_hidden_dim, cfg.projector_dim)

    def encode(self, waveform: torch.Tensor, acg: torch.Tensor) -> Dict[str, torch.Tensor]:
...
def augment_waveform(x: torch.Tensor, cfg: Config) -> torch.Tensor:
    ...
    if cfg.waveform_amplitude_jitter > 0:
        y *= 1 + cfg.waveform_amplitude_jitter * torch.randn(len(y), 1, 1, device=y.device)
    if cfg.waveform_noise_std > 0:
        y += cfg.waveform_noise_std * torch.randn_like(y)
    if cfg.waveform_time_shift > 0:
        shifts = torch.randint(-cfg.waveform_time_shift, cfg.waveform_time_shift + 1, (len(y),), device=y.device)
        y = torch.stack([torch.roll(v, int(s), dims=-1) for v, s in zip(y, shifts)])
    if cfg.waveform_channel_mask_probability > 0:
        mask = torch.rand(y.shape[:2], device=y.device) < cfg.waveform_channel_mask_probability
        y = y.masked_fill(mask[..., None], 0)
    if cfg.waveform_time_mask_probability > 0:
        mask = torch.rand((len(y), y.shape[-1]), device=y.device) < cfg.waveform_time_mask_probability
        y = y.masked_fill(mask[:, None, :], 0)
...
def waveform_reconstruction_loss(pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
    return (
        F.mse_loss(pred, target)
        + 0.5 * F.mse_loss(pred[..., 1:] - pred[..., :-1], target[..., 1:] - target[..., :-1])
        + 0.25 * F.mse_loss(pred[:, 1:] - pred[:, :-1], target[:, 1:] - target[:, :-1])
        + 0.5 * ((1 + 4 * target.abs()) * (pred - target).square()).mean()
    )


def waveform_morphology_loss(pred: torch.Tensor, target: torch.Tensor, cfg: Config) -> torch.Tensor:
    ...
    pmin, ptmin = soft_min(p); tmin, ttmin = soft_min(t)
    pmax, ptmax = soft_max(p); tmax, ttmax = soft_max(t)
    ...
    return (
        F.smooth_l1_loss(pmin, tmin) + F.smooth_l1_loss(pmax, tmax)
        + 0.5 * F.smooth_l1_loss(ptmin, ttmin) + 0.5 * F.smooth_l1_loss(ptmax, ttmax)
        + F.mse_loss(p_smooth[:, 1:] - p_smooth[:, :-1], t_smooth[:, 1:] - t_smooth[:, :-1])
    )


def acg_reconstruction_loss(pred: torch.Tensor, target: torch.Tensor, cfg: Config) -> torch.Tensor:
    ...
    p_mass = pred.sum(-1); t_mass = target.sum(-1)
    ...
    return value_loss + 0.5 * temporal_loss + 0.15 * row_loss + 0.1 * mass_loss + 0.1 * centroid_loss


def latent_scale_loss(z_wave: torch.Tensor, z_acg: torch.Tensor, cfg: Config) -> torch.Tensor:
    losses = []
    for z in (z_wave, z_acg):
        mean = z.mean(0)
        std = z.std(0, unbiased=False)
        losses.append(mean.square().mean() + (std - cfg.raw_latent_std_target).square().mean())
...
def vicreg_loss(z1: torch.Tensor, z2: torch.Tensor, cfg: Config):
    ...
    var = 0.5 * (F.relu(cfg.vicreg_std_target - s1).mean() + F.relu(cfg.vicreg_std_target - s2).mean())
    z1c = z1 - z1.mean(0); z2c = z2 - z2.mean(0)
    den = max(len(z1) - 1, 1)
    c1 = z1c.T @ z1c / den; c2 = z2c.T @ z2c / den
    cov = (off_diagonal(c1).square().sum() + off_diagonal(c2).square().sum()) / (2 * z1.shape[1])
