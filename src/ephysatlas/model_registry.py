...
union keyed on ``task`` that normalises every model family into one shape so a single load path
...
(:func:`verify_checksums`). The code that *writes* manifests, checksums and release directories
lives in ``paper-ephys-atlas`` alongside the training scripts: producing a model is a paper-side
concern, while this package is the public inference API.
"""
...
# silently dropped at publish time fails loudly instead of surfacing later as a confusing load
# error. The paper-side publisher writes this file; the load path only re-checks it.
MODEL_CHECKSUM_FILE = "checksums.json"
# checksums.json names which of iblutil's file hashers produced its digests. Verification resolves
# that name through this whitelist so an untrusted manifest cannot make us call an arbitrary
# attribute -- the paper-side writer and this verifier share the one function per algorithm, so
# their digests match by construction. These detect accidental corruption, not tampering (nothing
# covers checksums.json itself), so a fast, short digest like sha1 is the natural default.
CHECKSUM_HASHERS = {
...
class HFModelSource:
    ...
    def fetch(
        ...
        # install must neither pay for it nor fail at import time.
...
def verify_checksums(path_model: Path, missing_ok: bool = True):
    ...
    the digest set by the paper-side writer.
