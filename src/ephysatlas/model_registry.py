...
Two backends fetch a model directory, both exposing ``fetch(model_id, revision, cache_dir) -> Path``:

* :class:`HFModelSource` -- Hugging Face Hub. Public repos need no authentication, which is what
  lets people outside the IBL use these models.
* :class:`S3ModelSource` -- the historical private bucket; needs an authenticated ONE client.

:func:`resolve_model` picks between them.

This module also builds ``ephysatlas_model.json``, the *publication contract*: it normalises every
model family into one shape so a single load path serves them all. It is a discriminated union --
a shared core plus ``inputs``/``outputs``/``config`` blocks interpreted according to ``task``:
...
Adding a family means registering it in :data:`MODEL_CLASS_TASKS` and :data:`TASK_BUILDERS` here,
plus ``regionclassifier.MODEL_LOADERS``, and writing the block builder and loader they point at.
The transport layer is model-agnostic, and ``load_model`` works from the manifest alone.
"""
...
# Which task a model belongs to, keyed on the MODEL_CLASS recorded in the manifest. Used by
# write_manifest to infer the task when a caller does not pass one explicitly (the region trainer
# and the repackage tool rely on it). One canonical class per family -- the encoders record the
# bare class name their trainers write; the classifier records the fully-qualified XGBoost class.
MODEL_CLASS_TASKS = {
...
class HFModelSource:
    ...
    def set_visibility(self, private: bool):
        ...
        logger.info(f"{self.repo_id} is now {'private' if private else 'PUBLIC'}")
    ...
    def upload(
        ...
        if self.repo_id is None:
            raise ValueError(
                "repo_id is required to upload; pass HFModelSource(repo_id=...)"
            )
...
class S3ModelSource:
    ...
    def fetch(
        ...
        if self.one is None:
            raise ValueError(
                "an authenticated ONE instance is required to fetch from S3"
            )
        if revision:
...
def resolve_model(
    ...
    if source not in backends:
        raise ValueError(
            f"unknown source {source!r}, expected one of {sorted(backends)}"
        )
    ...
    for backend in backends[source]():
        try:
            ...
            continue
        # Every published model ships checksums.json, so require it on the load path: a fetch
        # that arrived without it is incomplete and must not be loaded.
        verify_checksums(path_model, missing_ok=False)
        return path_model
    raise ValueError(
        f"could not resolve model {model_id!r}. Tried -- " + " | ".join(errors)
    )
...
def _blocks_region_classification(meta: dict, path_model: Path) -> dict:
    ...
    fold_dirs = (
        sorted(p.name for p in folds_root.glob("FOLD*")) if folds_root.exists() else []
    )
    # Listing the fold directories outright means callers never have to guess a naming pattern
    # or a fold count.
...
def _blocks_spatial_encoding(meta: dict, path_model: Path) -> dict:
    ...
    weights = (
        ...
        if path_model.joinpath(ENCODER_WEIGHTS_FILE).exists()
        else (
            "SE_model.pth"
            if path_model.joinpath("SE_model.pth").exists()
            else ENCODER_WEIGHTS_FILE
        )
    )
...
def write_manifest(
    ...
    *,
    ...
    tool) passes the values directly.
    ...
    if task not in TASK_BUILDERS:
        raise ValueError(
            f"no builder for task {task!r}; known: {sorted(TASK_BUILDERS)}"
        )
    ...
    path_model.joinpath(MODEL_MANIFEST_FILE).write_text(
...
def validate_artifacts(path_model: Path, index: dict = None) -> bool:
    ...
    if missing:
        raise FileNotFoundError(
            f"{path_model.name}: manifest artifacts missing on disk: "
            + "; ".join(missing)
        )
    return True
...
def validate_feature_order(
    features, recorded_digest=None, current_features=None
) -> bool:
