...
def _architecture(path_model: Path, manifest: dict, state: dict) -> dict:
    ...
    Two sources: the manifest's ``config.architecture`` (into which the caller has already
    folded any ``architecture`` key the checkpoint carried), and the shapes of the normalisation
    buffers, which pin ``f_ctx`` and ``f_ephys`` even when nothing else says so.
    """
...
def _load_inpainting_encoder(path_model: Path, manifest: dict = None):
    ...
    Handles both checkpoint shapes in circulation: a bare ``state_dict`` whose architecture the
    manifest describes, and the wrapped ``{"model_state": ..., "architecture": {...}}`` dict.
    ...
    checkpoint = torch.load(
        path_model.joinpath(weights), map_location="cpu", weights_only=False
    )
...
def build_neighbor_bank(path_model: Path, df, manifest: dict, model=None) -> Path:
    ...
    if missing:
        raise KeyError(
            f"the bank source table is missing {len(missing)} column(s): {missing}"
        )
    if model is None:
    ...
    logger.info(
        f"wrote {out} with {xyz.shape[0]} channels from {len(set(pid))} insertions"
    )
    return out
...
class SpatialEncoder:
    ...
    def __init__(self, path_model, index: dict = None, device=None):
        ...
        if self.index is None:
            raise FileNotFoundError(
                f"{self.path_model} has no {model_registry.MODEL_MANIFEST_FILE}; its manifest is "
                f"the only record of the encoder's neighbourhood and context settings."
            )
    ...
    def context_dir(self) -> Path:
        ...
        were published at the model-dir root or under a ``context/`` subdirectory. A caller building
        its own ``ContextAtlasManager`` should read this rather than assume a fixed subdirectory.
        """
    ...
    def _standardised_context(self, xyz_m: np.ndarray) -> np.ndarray:
        ...
        pack = manager.sample_context_numpy_m(
            mirror_xyz_to_left(xyz_m.copy()), mode="clip"
        )
        ctx = np.concatenate([pack["cell_pc"], pack["gene_pc"]], axis=1).astype(
            np.float32
        )
    ...
    def _neighbours(self, xyz_m: np.ndarray, pids: np.ndarray):
        ...
        for i, cand in enumerate(candidates):
            ...
            if cand.size > m_max:
                order = np.argsort(
                    np.sum((bank["xyz"][cand] - xyz_m[i][None, :]) ** 2, axis=1),
                    kind="stable",
                )
