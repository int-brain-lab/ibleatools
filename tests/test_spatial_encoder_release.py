...
``training/train_spatial_encoder.py`` writes the canonical files at the model-dir root, builds the
neighbour bank, and writes the manifest directly -- no ``meta.yaml``, no ``encoding_models/``
subdirectory. This locks that contract (change sets A4/B5/B6) on a tiny seeded random-init model,
so it runs in seconds on CPU without the real W26 data or the full training pipeline.
...
def _load_trainer():
    """Import ``training/train_spatial_encoder.py`` by path (training/ is not on sys.path)."""
    path = Path(__file__).resolve().parents[1] / "training" / "train_spatial_encoder.py"
    spec = importlib.util.spec_from_file_location("train_spatial_encoder", path)
...
class TestSpatialReleaseLayout(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for name in ("agea_vol_pca.npy", "merfish_vol_pca.npy"):
            np.save(self.path_model.joinpath(name), np.zeros((2, 2, 2, 3), dtype=np.float32))
        ...
        self.bank_feat = np.arange(3 * len(FEATURES), dtype=np.float32).reshape(3, len(FEATURES))
    ...
    def test_writes_canonical_meta_free_manifest(self):
        ...
        self.trainer.write_spatial_release(
            self.path_model, self.model, self.meta, self.bank_xyz, self.bank_feat, self.bank_pid
        )
        # The scaffold is gone: no meta.yaml in the model output.
...
@unittest.skipUnless(_HAVE_DATA, f"needs {_W26} and the context volumes in {_SE_SOURCE}")
class TestSpatialReleaseLoadsAndPredicts(unittest.TestCase):
    ...
    def setUp(self):
        ...
        features = [str(f) for f in yaml.safe_load(_SE_SOURCE.joinpath("meta.yaml").read_text())["FEATURES"]]
        ...
        self.meta = dict(
            RANDOM_SEED=0, VINTAGE="2026_W26", MODEL_CLASS="NeighborInpaintingModel",
            FEATURES=features, F_CTX=f_ctx, D_MODEL=32, NHEAD=4, DEPTH=2, DROP=0.0,
            RADIUS_UM=500.0, M_MAX=8, ALLOW_SAME_PROBE=False, N_CELL_PCS=50, N_GENE_PCS=50,
        )
