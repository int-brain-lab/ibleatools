...
checkpoints, the encode/reconstruct/components/assign contract, output shapes and determinism --
...
class TestUnitEncoderWrapper(unittest.TestCase):
    ...
    def test_latents_reads_a_cached_atlas_and_standardises(self):
        # The atlas arrays are read from <cache>/arrays, prepared outside this package.
        # With the cache pre-populated, latents() must read it and encode it.
    ...
    def test_uncached_atlas_raises_and_points_at_the_prepare_step(self):
        # Preparing the arrays is training machinery that lives outside this package, so an
        # uncached atlas must fail loudly here rather than pulling multi-GB from S3 itself.
        cache = self.tmp.joinpath("empty_cache")
        with self.assertRaises(FileNotFoundError) as ctx:
            self._encoder().latents(cache_dir=cache)
        self.assertIn("prepare_latest_cells_encoder_data", str(ctx.exception))
