...
The code that *writes* manifests, checksums, splits and release directories lives in
``paper-ephys-atlas`` alongside the training scripts, and is tested there.
"""
...
class TestLoadModelDispatch(unittest.TestCase):
    ...
    def test_manifest_takes_precedence_over_meta_yaml(self):
        ...
        # contract and wins. The fixture writes no meta.yaml, so stage one here (with a wrong
        # MODEL_CLASS) to prove the manifest overrides it.
        legacy = dict(fixtures.region_meta())
    ...
    def test_manifest_alone_is_sufficient(self):
        # The current layout: a manifest and no meta.yaml. The manifest must be enough on its
        # own, and model_info stays meta-shaped for infer_regions callers.
...
class TestMetaFreeFolds(unittest.TestCase):
    ...
    def test_folds_carry_no_meta_yaml(self):
        # The fixture writes no meta.yaml at any level; folds hold model.ubj alone.
        self.assertFalse(self.path_model.joinpath("meta.yaml").exists())
...
class TestRegionClassifier(unittest.TestCase):
    ...
    def test_validate_feature_order_can_also_compare_against_caller_features(self):
        # A family with a module-level FEATURE_LIST (the spatial encoder) makes code-vs-release
        # drift meaningful; the classifier takes its list from the manifest, so this is optional.
...
class TestVerifyChecksums(unittest.TestCase):
    ...
    the bytes that were published? These test the load-path *verifier*; the writer lives with the
    publishing code in ``paper-ephys-atlas``.
    """
