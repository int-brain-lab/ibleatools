...
from ephysatlas.feature_computation import load_data_from_files, add_target_coordinates, compute_features_from_raw
    ...
    def test_compute_features_from_raw_with_destriped_files(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if "lf" in features_to_compute:
                expected_columns.extend(["rms_lf", "psd_delta", "psd_theta", "psd_alpha", "psd_beta", "psd_gamma", "psd_lfp"])
            if "ap" in features_to_compute:
                expected_columns.extend(["rms_ap", "cor_ratio"])
            
            for col in expected_columns:
                self.assertIn(col, result_df.columns, f"Expected column '{col}' not found in result")
            ...
            self.assertTrue(np.array_equal(result_df["channel"].values, np.arange(n_channels)))
