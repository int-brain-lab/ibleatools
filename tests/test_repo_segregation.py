...
This repo is the PUBLIC inference API: no training scripts, no figure production. What it holds
obeys a one-way dependency rule:
...
Run in a **subprocess** on purpose: the pytest process that runs the rest of the suite imports
the torch-heavy modules (``test_unit_encoder``), so this process's ``sys.modules`` is already
polluted. A clean interpreter is the only way to see what the public surface *alone* pulls in.
"""
